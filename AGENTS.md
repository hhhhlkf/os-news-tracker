...
- Root `docker-compose.yml` is permanently retired. Never run a bare `docker compose` command: development must use `docker compose -f docker-compose.dev.yml ...`; formal release must use `docker compose -p <release-project> -f docker-compose.production.yml ...`.
- Treat the Compose project and working directory as part of the environment identity. The currently running formal stack is launched from `/data/workspace/os-news-tracker` on the external `os-news-tracker_default` network and reaches its `db` service there (currently backed by `os-news-tracker_pgdata_dev`). The `/data/workspace/os-news-tracker-dev` development stack declares the distinct external `os-news-tracker_pgdata` volume. Do not infer database sharing from a volume name, and never create, switch, or migrate a production database volume without explicit authorization.
...
- A production release starts by committing and pushing all intended local changes. Build the exact release commit from a dedicated clean worktree; never build production images from a dirty development worktree.
- Run release Compose commands from `/data/workspace/os-news-tracker`, the production operator checkout. Create or retain a release worktree for the exact commit and set an absolute `PRODUCTION_WORKTREE` explicitly. Do not rely on `docker-compose.production.yml`'s default `./.worktrees/production-b4b067c`: it is a historical fallback and does not identify the current release.
- Production is image-based and does not hot-reload application source. Build and replace only `backend-prod` and `frontend-prod`; do not recreate the database, remove persistent volumes, or run a database migration unless the user explicitly authorizes it. The production runtime intentionally bind-mounts the approved connector-artifact directory and Nginx configuration and shares the WeChat, attestation, and checkpoint volumes; preserve them during releases.
- Before replacement, confirm that no process-local Discovery or manual fetch is running and identify the currently running production pair. Stop that pair without removing it, then start the new pair with a release-specific Compose project name on the shared external network. This prevents duplicate schedulers while preserving immediate rollback.
- Verify the homepage and API health through the new pair. Retain only the new running pair and the immediately previous stopped pair. Remove older production containers only after health checks pass. Never delete production volumes, connector data, networks, or approved runtime images during this cleanup.
- The executable runbook, including environment reload and rollback commands, is `docs/deployment/deployment-runbook.md`.
