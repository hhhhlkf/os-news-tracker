...
def test_detect_source_returns_detect_result(client):
    with patch("app.sources.source_management.detect_source", return_value=_rss_detect_result()) as detect:
...
def test_detect_source_unable_returns_422(client):
    with patch("app.sources.source_management.detect_source", side_effect=SourceDetectionError("nope")):
...
def test_create_source_redetects_and_persists_news_source(client):
    ...
    with patch("app.sources.source_management.detect_source", return_value=detect_result):
        response = client.post(
            "/sources",
            json={
...
def test_create_page_monitor_auto_discovers_html_list_selectors(client):
    ...
    with patch("app.sources.source_management.discover_html_list_source", return_value=discovery) as discover:
        response = client.post(
            "/sources",
            json={
...
def test_delete_source_cleans_related_items(client):
    session = client.app.dependency_overrides[get_db]()
