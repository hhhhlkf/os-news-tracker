...
def _page(url="https://a.com/1"):
    return RawPage(url=url, guessed_topic="kernel", title="Title", content="Content about eBPF scheduler")


def _make_llm(score=7):
    llm = MagicMock()
    llm.complete.return_value = json.dumps({
        "score": score, "reason": "relevant", "relevant_topic": "kernel",
...
async def test_site_memory_hit_skips_llm():
    ...
    assert len(results) == 1
    mock_llm.complete.assert_not_called()  # LLM not called due to cache hit
```
...
class QualityWorkerPool:
    ...
    async def assess_all(
        ...
        async def assess_one(page: RawPage) -> QualifiedPage | None:
            async with sem:
                ...
                if result.verdict == "discard":
                    logger.info("quality_pool: discard %s (score=%d)", page.url, result.score)
                    return None
        ...
        logger.info("quality_pool: %d/%d pages passed", len(qualified), len(pages))
        return qualified
```
