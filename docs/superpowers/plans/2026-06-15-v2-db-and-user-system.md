# V2 Database Migration & User System — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add all V2 database tables via a single Alembic migration, then implement JWT-based auth (register/login/me) with automatic user_profiles initialization.

**Architecture:** New ORM models added to `app/models.py`. New `app/auth.py` for JWT + password hashing. New `app/api/auth_routes.py` for auth endpoints. `app/api/deps.py` updated with `get_current_user` dependency. All existing tables and code are untouched.

**Tech Stack:** Python 3.11, FastAPI, SQLAlchemy 2.0, Alembic, `python-jose[cryptography]`, `passlib[bcrypt]`

**Spec:** `docs/superpowers/specs/2026-06-15-v2-complete-design.md` §3, §8
...
| File | Action | Notes |
|------|--------|-------|
| `backend/pyproject.toml` | Modify | Add `python-jose[cryptography]`, `passlib[bcrypt]` |
| `backend/app/models.py` | Modify | Add 8 new ORM models |
| `backend/app/enums.py` | Modify | Add `agent_crawl` to SourceType, `agent_enriched` to ItemStatus |
| `backend/app/auth.py` | Create | JWT helpers + password hashing |
| `backend/app/api/auth_routes.py` | Create | /auth/register, /auth/login, /auth/me, /auth/me (PUT) |
| `backend/app/api/deps.py` | Modify | Add `get_current_user` dependency |
| `backend/app/api/main.py` | Modify | Register auth_routes router |
| `backend/alembic/versions/v2_add_all_tables.py` | Create | Single migration for all V2 tables |
| `backend/tests/unit/test_auth.py` | Create | JWT + password unit tests |
| `backend/tests/integration/test_auth_api.py` | Create | Register/login/me integration tests |
...
In `backend/app/enums.py`, add `agent_crawl` to `SourceType` and `agent_enriched` to `ItemStatus`. The exact change depends on current content — add alongside existing values:
...
class SourceType(str, Enum):
    ...
    SEARCH = "search"
    AGENT_CRAWL = "agent_crawl"   # ← add this
...
class ItemStatus(str, Enum):
    ...
    AGENT_ENRICHED = "agent_enriched"   # ← add this
```
...
Expected: `agent_crawl agent_enriched`

---

### Task 3: Add V2 ORM Models

**Files:**
- Modify: `backend/app/models.py`

- [ ] **Step 1: Add all V2 models at the end of `backend/app/models.py`**
...
**Files:**
- Create: `backend/alembic/versions/v2_add_all_v2_tables.py`
...
Expected: creates a new file under `alembic/versions/`.

- [ ] **Step 2: Review and run the migration**
...
Expected: migration runs without error. Verify new tables:

```bash
psql $DATABASE_URL -c "\dt" | grep -E "users|user_profiles|digests|agent_"
```

Expected: lists `users`, `user_profiles`, `user_item_scores`, `user_item_interactions`, `digests`, `agent_source_configs`, `agent_site_memory`, `agent_crawl_runs`.
...
### Task 5: Auth Helpers (`app/auth.py`)
...
### Task 6: Update `deps.py` with `get_current_user`
...
- [ ] **Step 3: Create `backend/app/api/auth_routes.py`**
...
In `backend/app/api/main.py`, import and include the new router:
