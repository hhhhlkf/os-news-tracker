...
在模板列表每条模板的尾部增加垃圾桶图标按钮：

- 图标位置：每条模板卡片尾部，和模板标题/摘要同一行右侧或卡片操作尾部
- 点击行为：调用 `DELETE /mail/templates/{template_id}`
- 删除结果：数据库中对应 `mail_templates` 记录被真正删除，不是仅前端隐藏
- 删除成功后：前端刷新模板列表；若当前正在查看该模板详情，则清空选中态或切到下一条可用模板
- 删除失败后：给出显式错误提示，不静默失败
