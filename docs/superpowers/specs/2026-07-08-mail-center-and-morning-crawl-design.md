...
### 5.6 邮件发送 Provider 设计

邮件系统采用 `provider` 抽象，但第一版必须是完整可用的邮件系统，不是仅停留在接口层。

推荐方案：

1. 上层业务统一调用邮件发送服务
2. 底层通过 `MailProvider` 抽象实现不同发信通道
3. 第一版只实现 `SMTP provider`

这样做的目的不是推迟邮件能力，而是避免将模板、预定发送、日志、失败处理这些上层业务强绑定到某一家邮件服务。
...
由于第一版采用的是“通用 SMTP 配置”，因此只要企业邮箱提供标准 SMTP 参数，就应当可以直接接入。
...
用于前端配置完成后的连通性校验，但这两项属于实现阶段细化内容，不影响当前整体架构。
...
1. SMTP 配置以系统级单例保存，还是支持未来扩展为多套发信配置
...
1. 先保留系统级单例 SMTP 配置，后续如确有需要再扩展多套配置
