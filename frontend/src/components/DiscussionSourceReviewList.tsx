...
function SourceRow({ source, rules, selected, busy, onToggle }: { source: { id: number; name: string }; rules: Array<{ source_id: number; rule_type: string; match_value: string }>; selected: boolean; busy: boolean; onToggle: (checked: boolean) => void }) {
  ...
  const ruleText = sourceRules.map((rule) => `${rule.rule_type}: ${rule.match_value}`).join(" · ");
  return (
    <div style={{ ...row, borderColor: selected ? "#b9d4ff" : "#eaecf0", background: selected ? "#f8fbff" : "#fff" }}>
      <input type="checkbox" aria-label={`选择 ${source.name}`} checked={selected} disabled={busy} onChange={(event) => onToggle(event.target.checked)} />
      <div style={rowMain} title={`${source.name}${ruleText ? `\n${ruleText}` : ""}`}>
        <div style={rowTitleLine}>
          <span style={rowTitle}>{source.name}</span>
          <span style={pendingBadge}>pending</span>
        </div>
        {ruleText ? <div style={rulesStyle}>{ruleText}</div> : null}
      </div>
    </div>
  );
}
...
const rulesStyle: CSSProperties = { marginTop: 2, color: "#475467", fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" };
