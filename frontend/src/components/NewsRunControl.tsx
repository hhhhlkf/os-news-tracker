...
export function toAbsoluteDateTime(value: string, endOfDay: boolean): string | null {
  ...
  // Emit an explicit UTC ISO-8601 string so the backend receives an
  // unambiguous point-in-time regardless of the browser's local timezone.
  // Previously we used new Date() which interprets date-time strings
  // without a timezone suffix as LOCAL time, causing the same date input
  // to produce different UTC timestamps depending on the user's timezone.
...
export function NewsRunControl(props: NewsRunControlProps) {
  ...
  useEffect(() => {
    ...
    // After a run finishes, reset form to defaults so user can start fresh.
    // Idle state means no run has happened — sync from status (defaults anyway).
      ...
      <div style={{ display: "grid", gap: 16 }}>
        ...
        >
          <div style={{ display: "grid", gap: 16 }}>
            ...
            {/* Time filter stats — show whenever available for observability */}
            {status?.time_filter_stats && (
              ...
              >
                <div>
                  <span style={{ fontWeight: 600 }}>时间命中</span>{" "}
                  {status.time_filter_stats.matched}
                </div>
                <div>
                  <span style={{ fontWeight: 600 }}>缺少发布时间</span>{" "}
                  {status.time_filter_stats.missing_published_at}
                </div>
                <div>
                  <span style={{ fontWeight: 600 }}>早于开始</span>{" "}
                  {status.time_filter_stats.before_start}
                </div>
                <div>
                  <span style={{ fontWeight: 600 }}>晚于结束</span>{" "}
                  {status.time_filter_stats.after_end}
                </div>
              </div>
            )}

            {/* Gap reason when not fulfilled */}
            {status?.gap_reason &&
              (status.state === "completed" || status.state === "failed" || status.state === "stopped") && (
                <div
                  style={{
